...
> **V4**: точечный релиз поверх V3. Закрыта дыра приватности в PIN-защите чатов (содержимое было видно за диалогом), починен persist порядка закреплённых чатов после перетаскивания, добавлен тумблер `bgWatchdog` против бага «Подключение…» на MIUI/HyperOS, восстановлен альбомный режим (сломан в 26.27.1 из-за форс portrait-lock), снят лимит 10 закреплённых чатов (безусловно, тумблер убран из UI). Весь функционал V3 сохранён.
...
В 26.27.1 был добавлен форс portrait-lock (полевой баг F1, 4PDA 2026-08-13: отсутствие `-land` layout-ресурсов), из-за которого приложение перестало поворачиваться в альбомную ориентацию. По запросу пользователя лок отменён.

Фикс: `MainActivity.y(Ljava/lang/Boolean;)V`, блок `:goto_84` — перед `if-eqz v3, :cond_89` инъецирован безусловный `const/4 v3, 0x1`, гарантирующий переход на `setRequestedOrientation(2=USER)` (следование сенсору) независимо от входного параметра. Экран видео (`VideoWebViewScreen`) со своим `SENSOR`/`PORTRAIT`-гейтом работает независимо и не затронут.

### 📌 Лимит закреплённых чатов снят безусловно
Ранее в моде был скрытый тумблер «Больше закреплённых чатов (до 10)», но даже при включении `AppFlags.pinLimit(I)I` продолжал возвращать сток-лимит 10 — тумблер не работал. По запросу лимит снят полностью и тумблер убран из UI.

Фикс: `pinLimit(I)I` теперь безусловно возвращает `Integer.MAX_VALUE` (`const v0, 0x7fffffff`), поле `unlockPinLimit` дефолтится в `true` и помечено `hidden` в `tumblers.yaml` (та же конвенция, что у `pinnedSort`) — строка тумблера не рендерится в настройках, но поле-якорь остаётся для `pinLimit()`. Затрагивает все 3 callsite `Lp2d;->g()I` (accessor максимума закреплённых чатов), переадресованных на `pinLimit()`. Лимит серверный дополнительно — сервер может сбросить лишние пины при синхронизации, это ограничение мод обойти не может.
...
| Проверка | Результат |
|---|---|
| `MainActivity.y()`: `const/4 v3, 0x1` безусловно перед `if-eqz v3, :cond_89` → ветка всегда ведёт к `setRequestedOrientation(2=USER)` | ✅ подтверждено |
| `AppFlags.pinLimit(I)I`: `const v0, 0x7fffffff` (корректная 32-битная кодировка, не `const/4`/`const/16`) | ✅ подтверждено |
| `unlockPinLimit` default=`true` (clinit + resetAllDefaults); строка тумблера отсутствует в `AppFlagsView` UI-рендере | ✅ подтверждено |
| Все 3 callsite `Lp2d;->g()I` переадресованы на `AppFlags.pinLimit(I)I` | ✅ подтверждено |
